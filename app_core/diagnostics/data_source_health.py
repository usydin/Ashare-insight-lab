...
def run_data_source_health_check(
    ...
    configured_health_check_symbol = manager.get_health_check_symbol() or default_test_symbol
    test_symbol = str(enabled_items[0].get("code", configured_health_check_symbol)).strip() if enabled_items else configured_health_check_symbol
    primary_source = manager.get_primary_source()
    fallback_order = manager.get_fallback_order()
    registered_sources = manager.list_sources()
    fallback_enabled = manager.is_fallback_enabled()
    local_cache_status = manager.inspect_local_cache(test_symbol)

    logger.info(
        "data-source health check started app=%s version=%s environment=%s developer=%s timeout=%ss test_symbol=%s primary_source=%s",
        APP_NAME_EN,
        VERSION,
        environment,
        DEVELOPER,
    ...
    sample_fetch = check_sample_fetch(test_symbol, timeout_seconds, logger, manager)
...
def check_sample_fetch(
    test_symbol: str,
    timeout_seconds: int,
    logger: Any,
    manager: DataSourceManager | None = None,
) -> dict[str, Any]:
    manager = manager or DataSourceManager()
    try:
        result = manager.fetch_stock_daily_history(
            test_symbol,
        ...
        normalized_dataframe = result.data["normalized_dataframe"]
        latest_trade_date = ""
...
def determine_health_status(
    ...
    if sample_fetch.get("status") == "success":
        if sample_fetch.get("fallback_used"):
            return "degraded"
        return "healthy"
    return "degraded"
...
def build_suggestions(
    ...
    if overall_status == "degraded" and sample_fetch.get("status") != "success":
        suggestions.append("示例股票采集失败，可先运行 doctor 再检查代理、网络和东方财富连通性。")
    if sample_fetch.get("status") == "success" and sample_fetch.get("fallback_used"):
        suggestions.append("在线数据源本次未直连成功，当前依赖本地缓存兜底；缓存仅用于降级读取，不代表实时行情。")
